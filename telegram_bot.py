...
DATE_FMT = "%d.%m.%Y"
ADMINS = {"5144039813", "476179186", "7455095273"}  # ID админа
USER_ID_TO_NAME = {
...
DIALOG_KEYS = [
    ...
    'repay', 'shift', 'report_period', 'admin_expense', 'custom_analytics_period', 'supplier_edit', 'seller_expense', 'supplier_edit', 'report_fix', 'new_task', 'markup_analysis', 'custom_abc_period'
]
...
# --- ЗАМЕНИТЕ get_current_payroll_period НА ЭТУ ФУНКЦИЮ ---
def get_payroll_period(offset: int = 0) -> tuple[dt.date, dt.date]:
    """
    Рассчитывает начальную и конечную дату зарплатного периода.
    offset = 0 -> текущий период
    offset = -1 -> предыдущий период
    """
    today = dt.date.today()
    
    # Определяем конечную дату ТЕКУЩЕГО периода
    if today.day > 24:
        # Мы уже в периоде, который закончится 24-го числа следующего месяца
        current_period_end_month = today.month + 1
        current_period_end_year = today.year
        if current_period_end_month > 12:
            current_period_end_month = 1
            current_period_end_year += 1
    else:
        # Мы еще в периоде, который закончится 24-го числа этого месяца
        current_period_end_month = today.month
        current_period_end_year = today.year

    # Применяем смещение в месяцах
    # Формула для корректного вычета месяцев
    target_year = current_period_end_year + (current_period_end_month + offset - 1) // 12
    target_month = (current_period_end_month + offset - 1) % 12 + 1

    end_date = dt.date(target_year, target_month, 24)
    
    # Начальная дата - всегда 25-е число предыдущего месяца
    start_date = (end_date.replace(day=1) - dt.timedelta(days=1)).replace(day=25)
...
def calculate_accrued_bonus(context: ContextTypes.DEFAULT_TYPE, seller_name: str, start_period: dt.date, end_period: dt.date, all_reports=None, all_salaries=None):
    ...
    # 1. Считаем начисленные бонусы за период
    total_accrued = 0
    bonus_days = []
    for row in all_reports:
        try:
            report_date = pdate(row[0])
            if start_period <= report_date <= end_period and row[1] == seller_name:
                total_sales = parse_float(row[4])
    ...
    # 2. Проверяем, была ли выплата именно за этот период
    is_paid = False
    period_str = f"за период {sdate(start_period)}-{sdate(end_period)}"
    for row in all_salaries:
        if len(row) > 4 and row[1] == seller_name and row[2] == "Выплата бонуса" and period_str in row[4]:
            is_paid = True
            break
...
async def confirm_payout(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    # Новый формат: confirm_payout_ИМЯ_СУММА_СМЕЩЕНИЕ
    _, _, seller_name, amount_str, offset_str = query.data.split('_')
    ...
    kb = [[
        InlineKeyboardButton("✅ Да, выплатить", callback_data=f"execute_payout_{seller_name}_{amount}_{offset_str}"),
        InlineKeyboardButton("❌ Отмена", callback_data=f"view_salary_{seller_name}_{offset_str}")
    ]]
...
def build_task_keyboard(tasks: list, is_admin: bool):
    """Строит клавиатуру со списком задач на сегодня."""
    kb = []
    # Сортируем: сначала важные (🔴), потом обычные (⚪️)
    tasks.sort(key=lambda x: x[2] == "Важный", reverse=True)

    for task in tasks:
        task_id, text, priority, _, assigned_to, _, status, who_done, _ = (task + [""] * 9)[:9]
        
        if status == "Выполнена":
            btn_text = f"✅ {text} ({who_done})"
            kb.append([InlineKeyboardButton(btn_text, callback_data=f"view_task_details_{task_id}")])
        else:
            priority_icon = "🔴" if priority == "Важный" else "⚪️"
            assigned_text = f" (кому: {assigned_to})" if is_admin else ""
            btn_text = f"{priority_icon} {text}{assigned_text}"
            kb.append([InlineKeyboardButton(btn_text, callback_data=f"mark_task_done_confirm_{task_id}")])
...
async def show_task_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    todays_tasks = []
    for task in tasks:
        if len(task) > 3 and (task_date := pdate(task[3].split()[0])) and task_date == today:
            if is_admin or (user_name and user_name in task[4]):
                todays_tasks.append(task)

    msg = f"<b>📋 Задачи на сегодня ({sdate(today)}):</b>"
    if not todays_tasks:
        msg += "\n\n<i>Для вас на сегодня задач нет.</i>"
...
async def handle_task_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [[InlineKeyboardButton(f"⚪️ {seller}", callback_data=f"toggle_task_seller_{seller}")] for seller in SELLERS]
...
async def toggle_task_seller(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    selected_sellers = context.user_data['new_task'].setdefault('sellers', [])
    if seller_name in selected_sellers:
        selected_sellers.remove(seller_name)
    else:
        selected_sellers.append(seller_name)
    
    kb = [[InlineKeyboardButton(f"✅ {s}" if s in selected_sellers else f"⚪️ {s}", callback_data=f"toggle_task_seller_{s}")] for s in SELLERS]
    kb.append([InlineKeyboardButton("✅ Далее", callback_data="task_sellers_done")])
    await query.message.edit_reply_markup(reply_markup=InlineKeyboardMarkup(kb))

async def show_task_priority_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Запрашивает приоритет задачи."""
    query = update.callback_query
    context.user_data['new_task']['step'] = 'priority'
    kb = [
        [InlineKeyboardButton("🔴 Важный", callback_data="task_priority_Важный")],
        [InlineKeyboardButton("⚪️ Обычный", callback_data="task_priority_Обычный")]
    ]
    await query.message.edit_text("Какой приоритет у этой задачи?", reply_markup=InlineKeyboardMarkup(kb))
...
async def show_task_date_calendar(update: Update, context: ContextTypes.DEFAULT_TYPE, year: int = None, month: int = None):
    ...
    query = update.callback_query
    if 'priority' not in context.user_data['new_task']:
        context.user_data['new_task']['priority'] = query.data.split('_')[-1]
...
async def save_new_task(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    row_data = [
        task_id, task_data['text'], task_data['priority'], run_time.strftime(f"{DATE_FMT} %H:%M"),
        ", ".join(task_data['sellers']), task_data['creator_id'], "Ожидает", "", job_id
    ]
...
async def execute_mark_task_as_done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    task_row = ws.row_values(cell.row)
    task_text, creator_id, job_id = task_row[1], task_row[5], task_row[8]

    ws.update_cell(cell.row, 7, "Выполнена")
    ws.update_cell(cell.row, 8, f"{who} в {dt.datetime.now(pytz.timezone('Europe/Kiev')).strftime('%H:%M')}")
    ...
    try:
        kb = [[InlineKeyboardButton("🗑️ Удалить это сообщение", callback_data="delete_this_message")]]
        await context.bot.send_message(
            chat_id=creator_id,
            text=f"✅ Задача \"{task_text}\" была выполнена {who}.",
            reply_markup=InlineKeyboardMarkup(kb)
        )
    except Exception as e:
        logging.error(f"Не удалось уведомить создателя задачи {creator_id}: {e}")

    await show_task_menu(update, context)

async def show_completed_task_details(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Показывает полную информацию о выполненной задаче."""
    query = update.callback_query
    task_id = query.data.split('_')[-1]
    ...
    text, priority, datetime_str, assigned_to, _, who_done, _ = (task_info + [""] * 8)[:8]
    ...
    msg = (f"<b>✅ Задача (Выполнена)</b>\n\n"
           ...
           f"<b>Срок:</b> {datetime_str}\n"
           f"<b>Исполнители:</b> {assigned_to}\n"
           f"<b>Выполнил:</b> {who_done}")

    await query.message.edit_text(msg, parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 К списку задач", callback_data="task_menu")]]))
...
async def send_task_notification(context: ContextTypes.DEFAULT_TYPE):
    ...
    task_info = next((row for row in tasks if row and row[0] == task_id), None)
    if not task_info or task_info[6] == "Выполнена":
        return

    text, priority, assigned_to_str = task_info[1], task_info[2], task_info[4]
...
async def daily_task_cleanup(context: ContextTypes.DEFAULT_TYPE):
    ...
    ws = GSHEET.worksheet(SHEET_TASKS)
    all_tasks = ws.get_all_values()[1:]
    
    rows_to_delete = []
    
    for i, row in enumerate(all_tasks, start=2):
        try:
            task_date = pdate(row[3].split()[0])
    ...
    get_cached_sheet_data(context, SHEET_TASKS, force_update=True)
    logging.info(f"TASK_CLEANUP: Удалено {len(rows_to_delete)} выполненных задач.")
...
async def execute_payout(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Получаем правильный период для комментария
    start_period, end_period = get_payroll_period(offset)
    period_str = f"за период {sdate(start_period)}-{sdate(end_period)}"
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # --- ФИНАЛЬНЫЙ БЛОК ДЛЯ ЗАДАЧ ---
            ...
            await show_task_priority_menu(update, context)
        elif data.startswith("task_priority_"):
            await show_task_date_calendar(update, context)
        elif data.startswith("task_cal_nav_"):
            parts = data.split('_'); year, month = int(parts[3]), int(parts[4])
        ...
        elif data.startswith("mark_task_done_confirm_"):
...
def main():
    ...
    kiev_tz = pytz.timezone('Europe/Kiev')
    job_queue.run_daily(daily_task_cleanup, time=dt.time(hour=0, minute=5, tzinfo=kiev_tz))
